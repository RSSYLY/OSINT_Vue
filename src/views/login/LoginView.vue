    ...
    submitLoginRequest() {
      ...
      if (loginType === 'useEmail') {
        ...
        if ( emailEle.reportValidity() && pwdEle.reportValidity()){
          loginInfo = {
            type: 'email',
            username_or_email: document.querySelector('#loginEmail').value,
        ...
        // 使用用户名的情况
        const phoneEle = document.querySelector('#loginUsername');
        const pwdEle = document.querySelector('#loginUsername-pwd');
        // 验证是否通过表单验证
        if ( phoneEle.reportValidity() && pwdEle.reportValidity()){
          // 合并json
          loginInfo = {
            type: 'username',
            username_or_email: document.querySelector('#loginUsername').value,
            password: document.querySelector('#loginUsername-pwd').value,
            rememberMe: rememberMe
          }
        } else {
          snackbar({
  ...
  <div class="container-right">
    <mdui-card class="card">
      ...
      <mdui-tabs value="useEmail" id="login-type">
        <mdui-tab value="useEmail" icon="email" inline>邮箱</mdui-tab>
        <mdui-tab value="usePhone" icon="perm_identity" inline>用户名</mdui-tab>
        ...
        <mdui-tab-panel slot="panel" value="usePhone">
          <div class="tab-panel-div">
            <mdui-text-field icon="perm_identity" label="用户名" type="number" id="loginUsername" required></mdui-text-field>
            <mdui-text-field icon="key" label="密码" type="password" toggle-password id="loginUsername-pwd" required></mdui-text-field>
          </div>
        </mdui-tab-panel>
      </mdui-tabs>
